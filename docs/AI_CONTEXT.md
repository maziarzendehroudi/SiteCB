...
| **BK-19** | Mise à jour du Design System public et transformation du PageEditor en éditeur visuel WYSIWYG in-situ (Drag & Drop, Cache-busting) | **Réalisé** |
...
- Choisir la page à modifier dans le menu déroulant supérieur.
- **Modifier le contenu visuellement :** La page s'affiche exactement comme sur le site public. Survolez un bloc pour faire apparaître les commandes, cliquez sur "✎ Éditer" pour modifier le texte in-situ, ou utilisez la poignée de glissement pour réorganiser l'ordre des blocs (Drag & Drop).
- Valider l'enregistrement : le service GitHub se charge de compiler les blocs en Markdown, de les encoder en UTF-8 et de générer automatiquement un commit sur le dépôt distant, déclenchant la mise à jour du site via le pipeline CI/CD.
